...
function Navbar({ hideSubNav }) {
  ...
  return (
    <header className="w-full bg-white flex flex-col sticky top-0 z-50 shadow-[0_4px_20px_rgba(0,0,0,0.03)] select-none border-b border-gray-100 font-sans antialiased">
      ...
      <div className="w-full h-[72px] px-4 md:px-6 flex items-center justify-between max-w-[1170px] mx-auto gap-4 lg:gap-6">
        ...
        >
          ...
          />
          <span className="text-[22px] font-black text-gray-900 tracking-tight hidden md:block">
            JioMart
          </span>
        </div>
        ...
        {!hideSubNav && (
          <div className="hidden xl:flex items-center bg-[#f0f4f9] rounded-full p-1 shrink-0 border border-gray-100/40">
            <button type="button" className="flex items-center gap-1.5 px-3.5 py-1 text-[12px] font-black italic text-gray-600 hover:text-black transition-colors cursor-pointer bg-transparent border-none outline-none">
              <svg className="w-3.5 h-3.5 text-[#0078ad]" fill="currentColor" viewBox="0 0 24 24">
                <path d="M13 2L3 14h7v8l11-12h-8l4-8z" />
              </svg>
              Quick
            </button>
            <button type="button" className="bg-[#0078ad] text-white px-4 py-1 rounded-full text-[12px] font-bold shadow-2xs cursor-pointer hover:bg-[#0c5273] transition-colors border-none outline-none">
              Shop all
            </button>
          </div>
        )}
        ...
        <div className="hidden lg:flex items-center gap-2.5 cursor-pointer group shrink-0 min-w-[170px] text-left">
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-1 text-[13px] font-bold text-gray-900 leading-none mb-1">
              Deliver to
              <svg className="w-3 h-3 text-[#0078ad] stroke-[3px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </div>
            <span className="text-[11px] font-semibold text-gray-400 leading-none truncate max-w-[130px]">
              Sagara, 577401
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 max-w-[440px] h-10 bg-[#f0f4f9] rounded-full flex items-center px-4 border border-transparent focus-within:border-gray-200/80 focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(0,120,173,0.03)] transition-all">
          <svg
        ...
        <div className="flex items-center gap-3.5 shrink-0 pl-2">
          ...
          {/* Offers (%) */}
          <button type="button" className="w-9 h-9 rounded-full bg-gray-900 hover:bg-black text-white flex items-center justify-center transition-colors cursor-pointer shadow-2xs border-none outline-none">
            <span className="font-bold text-[14px] leading-none mb-[0.5px]">%</span>
          </button>
          ...
          {user?.role !== "admin" && (
            <button
              ...
              className="w-9 h-9 text-gray-700 hover:text-black hover:bg-gray-50 rounded-full transition-all cursor-pointer flex items-center justify-center border-none outline-none"
            >
          ...
          {user?.role !== "admin" && (
            <button
              ...
              className="w-9 h-9 text-gray-700 hover:text-black hover:bg-gray-50 rounded-full transition-all relative cursor-pointer flex items-center justify-center border-none outline-none"
            >
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
                <path d="M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zM1 2v2h2l3.6 7.59-1.35 2.45c-.16.28-.25.61-.25.96 0 1.1.9 2 2 2h12v-2H7.42c-.14 0-.25-.11-.25-.25l.03-.12.9-1.63h7.45c.75 0 1.41-.41 1.75-1.03l3.58-6.49A1.003 1.003 0 0020 4H5.21l-.94-2H1zm16 16c-1.1 0-1.99.9-1.99 2s.89 2 1.99 2 2-.9 2-2-.9-2-2-2z"/>
              </svg>
              <span className="absolute top-1 right-1 bg-[#e80018] text-white text-[9px] font-bold w-[16px] h-[16px] rounded-full flex items-center justify-center border border-white shadow-2xs">
                {cartCount}
              </span>
            </button>
          )}

          {/* User Profile Dropdown Overlay Group */}
            ...
            {!token ? (
              ...
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-gray-200 bg-white hover:bg-gray-50 transition-all cursor-pointer outline-none"
                >
                  <span className="text-[12px] font-bold text-gray-800 max-w-[80px] truncate">
                    {user?.name}
                  </span>
                  <svg className={`w-3 h-3 text-gray-400 transition-transform stroke-[2.5] ${showDropdown ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                ...
                {showDropdown && (
                  <div className="absolute right-0 mt-2.5 w-56 bg-white border border-gray-100 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.08)] z-50 overflow-hidden py-1">
                    <div className="px-4 py-3 border-b border-gray-100 text-left bg-gray-50/40">
                    ...
                    {user?.role === "admin" && (
                      <button
                        ...
                        className="w-full text-left px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 hover:text-black transition-colors cursor-pointer border-none outline-none"
                      >
                        Admin Dashboard
                      </button>
                    )}
                    ...
                    <button
                      ...
                      className="w-full text-left px-4 py-2.5 text-sm font-bold text-red-600 hover:bg-red-50 transition-colors border-t border-gray-100 cursor-pointer border-none outline-none"
                    >
                      Logout
                    </button>
                  </div>
      ...
      {!hideSubNav && (
        <div className="w-full bg-white border-t border-gray-100 hidden md:block">
          <div className="max-w-[1170px] w-full mx-auto flex items-end overflow-x-auto scrollbar-none h-11 px-2">
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-5 rounded-t-xl font-bold text-[13px] h-9 transition-all cursor-pointer border-none outline-none ${
                      isTabActive
